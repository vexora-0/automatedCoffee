...
import { LineChart, BarChart3, PieChart, TrendingUp, Clock, Package, CreditCard, Coffee, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
const TabsList = ({ className = '', children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={`inline-flex h-12 items-center justify-center rounded-lg bg-gray-100 p-1 text-slate-700 ${className}`} {...props}>
    {children}
  </div>
);
...
const TabsTrigger = ({ value, className = '', children, ...props }: {
  ...
  <button className={`inline-flex items-center justify-center whitespace-nowrap rounded-md px-4 py-2.5 text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm ${className}`} {...props}>
    {children}
  </button>
);
...
const SelectTrigger = ({ className = '', children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={`flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground ${className}`} {...props}>
    {children}
  </div>
);
...
const SelectItem = ({ value, children, ...props }: { 
  ...
  <div className="relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none hover:bg-accent hover:text-accent-foreground" {...props}>
    {children}
  </div>
);
...
const Alert = ({ variant = 'default', className = '', children, ...props }: { 
  ...
  <div className={`relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground ${variant === 'destructive' ? 'border-destructive/50 text-destructive dark:border-destructive [&>svg]:text-destructive' : ''} ${className}`} {...props}>
    {children}
  </div>
);
...
const Badge = ({ variant = 'default', className = '', children, ...props }: {
  ...
  return (
    <div className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${variantClasses[variant]} ${className}`} {...props}>
      {children}
    </div>
  );
};
...
const DatePickerWithRange = ({ date, onDateChange, className }: DatePickerProps) => (
...
const SalesChart = ({ data, timeFrame }: { data: any[]; timeFrame: string }) => (
  <div className="w-full h-full flex items-center justify-center bg-gray-50 rounded-lg">
    <div className="text-center">
      <div className="text-lg font-medium">Sales Chart</div>
      <div className="text-sm text-gray-500">{timeFrame} data visualization</div>
    </div>
  </div>
);
...
const SalesOverTimeChart = ({ data, dataKey, color, timeFrame }: { 
  data: any[]; 
  dataKey: string; 
  color: string;
  timeFrame: string;
  ...
  <div className="w-full h-full flex items-center justify-center bg-gray-50 rounded-lg">
    <div className="text-center">
      ...
      <div className="text-sm text-gray-500">Showing {dataKey} data for {timeFrame}</div>
    </div>
  </div>
);
...
const MachinePerformanceCard = ({ 
  ...
  <Card>
    <CardHeader>
      ...
      <CardDescription>
        {selectedMachine === 'all' ? 'All machines performance' : 'Single machine metrics'}
      </CardDescription>
    </CardHeader>
      ...
      {isLoading ? (
        ...
        <div className="h-full w-full flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500">Machine data visualization</div>
          </div>
        </div>
      )}
    </CardContent>
...
const PopularProductsTable = ({ products }: { products: Product[] }) => (
  <div className="overflow-x-auto">
    <table className="w-full">
      <thead>
        <tr>
          <th className="text-left py-2 px-2 text-sm font-medium text-gray-500">Product</th>
          <th className="text-left py-2 px-2 text-sm font-medium text-gray-500">Units Sold</th>
          <th className="text-left py-2 px-2 text-sm font-medium text-gray-500">Revenue</th>
        </tr>
      </thead>
      <tbody>
...
const apiService = {
  async get<T>(url: string, options?: { params?: any }): Promise<{ success: boolean; data: T }> {
    try {
...
function generateMockData<T>(url: string): T {
  ...
  if (url.includes('/sales/product')) {
    return Array.from({ length: 8 }, (_, i) => ({
      id: `p${i+1}`,
      name: ['Espresso', 'Latte', 'Cappuccino', 'Americano', 'Mocha', 'Flat White', 'Cold Brew', 'Macchiato'][i],
      unitsSold: Math.floor(Math.random() * 200) + 50,
      revenue: Math.floor(Math.random() * 20000) + 5000,
      category: ['Coffee', 'Coffee', 'Coffee', 'Coffee', 'Coffee', 'Coffee', 'Cold Drinks', 'Coffee'][i]
    })) as unknown as T;
  } 
...
export default function AnalyticsPage() {
  ...
  const [machinePerformance, setMachinePerformance] = useState<any | null>(null);
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      ...
      try {
        ...
        if (!salesResponse || !productResponse || !categoryResponse || !revenueResponse) {
          setError('Some data couldn\'t be loaded from the server. Displaying demo data.');
        }
      } catch (error) {
        console.error('Error fetching analytics data:', error);
        setError('Unable to load analytics data. Using demo data for visualization purposes.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Fetch functions
  const fetchSalesData = async (params: QueryParams): Promise<SalesData | null> => {
  ...
  const fetchProductSalesData = async (params: QueryParams): Promise<Product[]> => {
    const response = await apiService.get<Product[]>('/analytics/sales/product', { params });
    return response.success ? response.data : [];
  };

  const fetchCategorySalesData = async (params: QueryParams): Promise<any[]> => {
    const response = await apiService.get<any[]>('/analytics/sales/category', { params });
    return response.success ? response.data : [];
  };

  const fetchRevenueData = async (params: QueryParams): Promise<RevenueData | null> => {
    const intervalParam = timeFrame === 'today' ? 'hourly' 
      : timeFrame === '7days' ? 'daily'
      : timeFrame === '30days' ? 'weekly'
      : 'daily';
    
    params.interval = intervalParam;
    
    const response = await apiService.get<RevenueData>('/analytics/revenue', { params });
    return response.success ? response.data : null;
  };

  const fetchPopularProductsData = async (params: QueryParams): Promise<Product[]> => {
    params.limit = 10;
    const response = await apiService.get<Product[]>('/analytics/products/popular', { params });
    return response.success ? response.data : [];
  };

  const fetchSalesOverTimeData = async (params: QueryParams): Promise<any[]> => {
    const intervalParam = timeFrame === 'today' ? 'hourly' 
      : timeFrame === '7days' ? 'daily'
      : timeFrame === '30days' ? 'weekly'
      : 'daily';
    ...
    const response = await apiService.get<any[]>('/analytics/sales/time', { params });
    return response.success ? response.data : [];
  };

  const fetchMachinePerformanceData = async (params: QueryParams): Promise<any> => {
    const endpoint = selectedMachine !== 'all' 
    ...
    <div className="container mx-auto p-6 space-y-8 bg-gray-50 min-h-screen">
      <div className="flex flex-col space-y-2">
        ...
        <p className="text-muted-foreground text-gray-500">
          Comprehensive insights and performance metrics for your coffee machines
          </p>
        </div>
      ...
      <Card className="bg-white shadow-md">
        <CardHeader className="pb-2">
          ...
          <CardDescription>Key metrics from all coffee machine operations</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard 
              title="Total Revenue" 
              value={revenueData ? `₹${revenueData.total.toLocaleString('en-IN')}` : "₹0"} 
              trend="up" 
            ...
            <StatCard 
              ...
              value={revenueData ? `₹${revenueData.average.toLocaleString('en-IN')}` : "₹0"} 
              trend="neutral" 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="col-span-1 md:col-span-2">
              <CardHeader>
                ...
                <CardDescription>
                  {timeFrame === 'today' ? 'Hourly sales for today' :
                   timeFrame === '7days' ? 'Daily sales for the past week' :
                   timeFrame === '30days' ? 'Weekly sales for the past month' :
                   'Custom period sales'}
                </CardDescription>
              </CardHeader>
              <CardContent className="h-80">
                {isLoading ? (
                  ...
                  <SalesChart 
                    data={timeFrame === 'today' ? salesData.hourly || [] : salesData.daily || []} 
                    timeFrame={timeFrame}
                  />
                ) : (
                  <div className="h-full w-full flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="col-span-1 md:col-span-2 bg-white shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  {timeFrame === 'today' ? 'Hourly sales breakdown' :
                   timeFrame === '7days' ? 'Daily sales for the past week' :
                   timeFrame === '30days' ? 'Weekly sales for the past month' :
