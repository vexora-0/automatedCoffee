...
import {
  useMachineStatus,
  useMachineTemperature,
  useMachineInventory,
} from "@/lib/websocket/socketContext";
...
export default function MachineStatus({ machineId }: MachineStatusProps) {
  ...
  const currentTemperature =
    temperature !== undefined ? temperature : machine.temperature_c;
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm">
                {new Date(machine.last_regular_service).toLocaleDateString()}
              </p>
            </div>
          </div>
        </CardContent>
